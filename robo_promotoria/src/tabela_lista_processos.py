...
def execute_process(options):
    ...
    personagens_cutoff = 2

    docu_totais = spark.sql(
        """
        SELECT docu_orgi_orga_dk_responsavel as orgao_dk, cldc_ds_classe as classe_documento, DOCU_NR_MP, DOCU_NR_EXTERNO, docu_tx_etiqueta, PCAO_DT_ANDAMENTO, TPPR_DESCRICAO, TPPR_DK, DOCU_DK
        ...
        JOIN {1}.tb_regra_negocio_processo ON cod_pct = cod_atribuicao AND classe_documento = docu_cldc_dk
    ...
    personagens = spark.sql(
        """
        SELECT docu_nr_mp, concat_ws(', ', collect_list(nm_personagem)) as personagens FROM
        (SELECT docu_nr_mp, CASE WHEN nr_pers = {1} THEN 'e outros...' ELSE pess_nm_pessoa END AS nm_personagem, nr_pers
        FROM (
            SELECT docu_nr_mp, pess_nm_pessoa, row_number() OVER (PARTITION BY docu_nr_mp ORDER BY pess_dk DESC) as nr_pers
            FROM DOCU_TOTAIS
    ...
    dts_ultimos_andamentos = spark.sql(
        """
        SELECT DOCU_NR_MP, MAX(PCAO_DT_ANDAMENTO) AS DT_ULTIMO
        FROM DOCU_TOTAIS
        GROUP BY DOCU_NR_MP
        """
    )
    ...
    lista_processos = spark.sql(
        """
        SELECT A.orgao_dk, A.classe_documento as cldc_dk, A.docu_nr_mp,
        ...
        GROUP BY A.orgao_dk, A.classe_documento, A.docu_nr_mp, A.docu_nr_externo, A.docu_tx_etiqueta, P.personagens, A.pcao_dt_andamento
        """
    )
