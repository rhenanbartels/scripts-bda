...
def create_regra_orgao(spark, options, nm_tipo, pacotes, cldc_dks, tppr_dks, days_past=30):
    ...
    date_old = date_now - timedelta(days=days_past)
    date_old = date_old if date_old >= datetime(2020, 5, 15) else datetime(2020, 5, 15)
    ...
    """.format(schema_exadata_aux, cldc_dks, pacotes, date_old, date_now))
    ...
    """.format(nm_tipo, days_past, nm_table_atuais, nm_table_anteriores, nm_table_acervo, schema_exadata_aux))
...
def create_regra_cpf(spark, options, nm_tipo, pacotes, cldc_dks, tppr_dks, days_past=30):
    schema_exadata = options['schema_exadata']
    schema_exadata_aux = options['schema_exadata_aux']
...
def execute_process(options):
    ...
    tppr_dks = ("(6549,6593,6591,6343,6338,6339,6340,6341,6342,7871,7897,7912,"
                "6346,6350,6359,6392,6017,6018,6020,7745,6648,6649,6650,6651,6652,6653,6654,"
                "6038,6039,6040,6041,6042,6043,7815,7816,6620,6257,6258,7878,7877,6367,6368,6369,6370,1208,1030,6252,6253,1201,1202,6254)")
    nm_table_1 = create_regra_orgao(spark, options, nm_tipo, pacotes, cldc_dks, tppr_dks, days_past)
